...
import { Image as ImageIcon, CheckCircle2, Loader2, ArrowLeft, ChevronDown } from 'lucide-react';
import { useSession } from 'next-auth/react';
import { uploadImage, updateProduct, getProductById, getProducts } from '@/lib/api';
...
export default function EditProductAdmin() {
  ...
  const loadProduct = useCallback(async () => {
    if (!id || !token) return;
    try {
      const product = await getProductById(id as string, token);
      setFormData({
        name: product.name,
        slug: product.slug,
  ...
  return (
    <div className="p-4 md:p-8 lg:p-12 max-w-5xl mx-auto">
      <div className="mb-10 flex items-center justify-between">
        <div>
          <Link href="/admin/produtos" className="text-zinc-400 hover:text-zinc-600 flex items-center gap-2 text-sm font-bold uppercase tracking-widest mb-4 transition-colors">
            <ArrowLeft className="w-4 h-4" /> Voltar para lista
          </Link>
          <h1 className="text-3xl md:text-4xl font-serif font-bold text-zinc-900">Editar Produto</h1>
          <p className="text-zinc-500 mt-2">Atualize as informações para manter seu catálogo em dia.</p>
        </div>
      </div>
      ...
      {isSubmitted ? (
        <div className="bg-white rounded-3xl p-12 shadow-sm border border-zinc-200 flex flex-col items-center justify-center text-center min-h-[400px]">
          ...
          <h2 className="text-3xl font-serif font-bold text-zinc-900 mb-4">Alterações Salvas!</h2>
          <p className="text-zinc-600 mb-8 max-w-md">O produto foi atualizado com sucesso. Redirecionando para a lista...</p>
        </div>
      ) : (
        <form onSubmit={handleSubmit} className="bg-white rounded-3xl p-6 md:p-10 shadow-sm border border-zinc-200">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
            ...
            <div className="lg:col-span-1">
              <label className="text-sm font-bold text-zinc-900 uppercase tracking-widest block mb-4">
                {imageFile ? 'Nova Imagem' : 'Imagem Atual'}
              </label>
              <div className="relative aspect-[4/5] w-full border-2 border-dashed border-zinc-300 hover:border-primary bg-zinc-50 rounded-2xl overflow-hidden transition-all group shadow-inner">
                ...
                {imagePreview ? (
                  <div className="relative w-full h-full">
                    ...
                    <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center pointer-events-none">
                      <div className="bg-white text-zinc-900 px-4 py-2 rounded-full font-bold text-xs flex items-center gap-2">
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-6 pointer-events-none">
                    ...
                    <p className="font-bold text-zinc-700 text-sm">Clique para alterar</p>
                    <p className="text-xs text-zinc-400 mt-1">PNG, JPG ou WebP (máx. 50 MB)</p>
                  </div>
                )}
              </div>
              {imageFile && (
                <p className="mt-3 text-xs text-primary font-bold flex items-center gap-2">
                  <CheckCircle2 className="w-3 h-3" /> Nova imagem selecionada
                </p>
              )}
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="flex flex-col gap-2 relative">
                  ...
                  <div className="relative">
                    ...
                    {isDropdownOpen && (
                      <div className="absolute top-full left-0 right-0 z-20 mt-2 bg-white border border-zinc-100 shadow-xl rounded-xl py-2 max-h-60 overflow-y-auto animated fadeInDown duration-200">
